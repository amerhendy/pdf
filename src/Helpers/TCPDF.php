...
use Amerhendy\Pdf\includes\TCPDF_CSS;
//============================================================+
// File name   : tcpdf.php
// Version     : 6.6.5
// Begin       : 2002-08-03
...
class TCPDF {
	...
	/**
	 * Array used to store the lengths of cache filfdefes.
	 * @protected
	 * @since 4.5.029 (2009-03-19)
	 */
	...
	protected function setPageFormat($format, $orientation='P') {
		...
		if (is_string($format)) {
			...
			$this->fwPt = $pf[0]; //width
			$this->fhPt = $pf[1]; //height
		} else {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					...
					if (defined(config('Amer.TCPDF.ThaiTopchars')) AND (config('Amer.TCPDF.ThaiTopchars') == true)) {
						...
						$this->setFontSubBuffer($this->CurrentFont['fontkey'], 'subsetchars', $this->CurrentFont['subsetchars']);
					} // end of config('Amer.TCPDF.ThaiTopchars')
